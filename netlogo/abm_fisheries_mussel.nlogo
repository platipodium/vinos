...
to setup-ports
    ...
    if  (item 18 row) < 9.16 [
      ...
      if not (length xy = 0) [
        create-ports 1 [
          ...
          set start-patch min-one-of patches with [depth > navigable-depth] [distance myself]
  ...
  while [ not file-at-end? ] [
    ...
    if  (item 18 row-f) < 9.16 [
      ...
      create-ports 1  [
      set kind "favorite-landing-port"      ;  some of these ports are not in Germany
      ...
      set landing-patch min-one-of patches with [depth > navigable-depth] [distance myself]
