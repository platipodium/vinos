...
def save_holidays(country, df):
    ...
    with open(filename, 'w') as fid:
        ...
        fid.write(f'; SPDX-FileCopyrightText: 2023 Helmholtz-Zentrum hereon GmbH\n')
        fid.write(f'; SPDX-License-Identifier: CC0-1.0\n')
...
def get_holidays(years):
    ...
    de = pd.DataFrame(index=de.keys(),data=de.values()).sort_index()
    ma = pd.DataFrame(index=ma.keys(),data=ma.values()).sort_index()
