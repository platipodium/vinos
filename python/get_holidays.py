...
def save_holidays(country, df):
    ...
    with open(filename, "w") as fid:
        ...
        fid.write(
            "; SPDX-FileCop{}yrightText: 2023-2024 Helmholtz-Zentrum hereon GmbH\n"
        )
        fid.write("; SPDX-Lic{}ense-Identifier: CC0-1.0\n")
        fid.write("Date,Holiday\n")
...
def get_holidays(years):
    ...
    de = pd.DataFrame(
        index=de.keys(), data=de.values(), columns=["Holiday"]
    ).sort_index()
    ma = pd.DataFrame(
        index=ma.keys(), data=ma.values(), columns=["Holiday"]
    ).sort_index()
