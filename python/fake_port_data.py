...
def write_data(fid, means=(0.9, 1.1, 0.8, 1.2)):
    ...
    fid.write("sub_grp,median,mean,sd,min,max\n")
    [
        fid.write(
            f"{i+1},{means[i]},{means[i]},{sfactors[i]*means[i]},{0.5*means[i]},{2*means[i]}\n"
        )
        for i in range(4)
    ]
...
def write_files():
    ...
    vardict = {
        ...
        "VE_KW": {
            "description": "Vessel engine power in kW",
            "means": (220, 220, 190, 220),
        },
        "distance_to_port": {
            "description": "Vessel distance from port in m",
            "means": (75, 125, 35, 250),
        },
        "triplength": {
            "description": "Vessel triplength in j",
            "means": (30, 45, 15, 65),
        },
        "LPUE": {"description": "Landings per unit effort", "means": (40, 45, 55, 5)},
        "total_fishinghours": {
            "description": "Vessel annual fishing in h",
            "means": (1600, 2200, 1000, 2100),
        },
    }
